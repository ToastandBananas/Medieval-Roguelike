...
public class Projectile : MonoBehaviour
{
    ...
    Unit shooter;
    Projectile_SO projectileScriptableObject;
    ...
    public void Setup(Projectile_SO projectile_SO, Unit shooter, Transform parentTransform, Action onProjectileBehaviourComplete)
    {
    ...
    Vector3 GetOffset(Unit shootingUnit, bool accountForAccuracy)
    {
        ...
        if (accountForAccuracy && random > shootingUnit.RangedAccuracy())
        {
    ...
    void OnTriggerEnter(Collider collider)
    {
        if (collider.isTrigger == false)
        {
            if (collider.CompareTag("Unit Body"))
            {
                ...
                if (unit != shooter)
                {
                    unit.TakeDamage(shooter.LeftHeldItem().GetDamage());
