...
public class GridSystemVisual : MonoBehaviour
{
    ...
    public void ShowGridPositionRange(GridPosition gridPosition, float minRange, float maxRange, GridVisualType gridVisualType)
    {
        ...
        for (int i = 0; i < path.allNodes.Count; i++)
        {
            ...
            Vector3 shootDir =  (gridPosition.WorldPosition() + (Vector3.up * selectedUnit.ShoulderHeight()) - ((Vector3)path.allNodes[i].position + (Vector3.up * selectedUnit.ShoulderHeight()))).normalized;
            if (Physics.SphereCast((Vector3)path.allNodes[i].position + (Vector3.up * selectedUnit.ShoulderHeight()), sphereCastRadius, shootDir, out RaycastHit hit, Vector3.Distance(selectedUnit.WorldPosition() + (Vector3.up * selectedUnit.ShoulderHeight()), (Vector3)path.allNodes[i].position + (Vector3.up * selectedUnit.ShoulderHeight())), UnitActionSystem.Instance.ActionObstaclesMask()))
    ...
    {
        if (UnitActionSystem.Instance.SelectedUnit() != null && TurnSystem.Instance.IsPlayerTurn() && UnitActionSystem.Instance.SelectedUnit().GetAction<MoveAction>().IsActive() == false)
            UpdateGridVisual();
    }
